...
const ChatProvider = (props) => {
    ...
    const cargarMensajes = () => {
        db.collection('chat').orderBy('fecha')
            //detectar base de datos en tiempo real
            .onSnapshot(query => {
               const arrayMensajes = query.docs.map(item => item.data()) //toda esta query hace el recorrido map y guarda en el array todos los mensajes en el chat
